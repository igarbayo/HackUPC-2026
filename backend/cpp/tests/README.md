...
| `test_exhausted_families_dispatched_on_tick` | All 4 pallet slots hold families A/B/C/D (1 box each, placed via direct delivery). The aisle contains only family X (belt empty, no A/B/C/D boxes anywhere). One `robot.tick()` must detect that A/B/C/D are exhausted (`countByFamily=0`, `reservedByFamily=0`), dispatch all four pallets, and open a new pallet for X. Verifies the step-1b exhaustion-dispatch path added to `tick()`. |
